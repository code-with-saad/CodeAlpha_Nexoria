...
| **Phase 3** | **Cart & Checkout (Stripe Test Mode)**<br>• `CartContext` with `localStorage` persistence (`nexoria_cart`)<br>• Cart quantity modification, removal, and live subtotal/tax computation<br>• Backend Order API (`POST /api/orders`, `GET /api/orders/myorders`, `GET /api/orders/:id`, `PUT /api/orders/:id/pay`)<br>• Stripe PaymentIntent integration (`POST /api/orders/create-payment-intent`)<br>• CheckoutPage with shipping address form & test card input<br>• OrderConfirmationPage & OrderHistoryPage with live receipt tracking | `In Progress` | Backend order routes & Stripe integration complete; CartContext, CartPage, CheckoutPage, OrderConfirmationPage, and OrderHistoryPage fully wired. |
